...
def add_log(text_line):
    for log_el in os.listdir(r'C:\lawer_bot\logs'):
        if log_el == 'log-{}.txt'.format(str(datetime.now() - timedelta(days=7)).split(' ')[0]):
            os.remove(r'C:\lawer_bot\logs/{}'.format(log_el))
    with codecs.open(r'C:\lawer_bot\logs/log-{}.txt'.format(datetime.now().date()), 'a', encoding='utf-8') as filewrite:
